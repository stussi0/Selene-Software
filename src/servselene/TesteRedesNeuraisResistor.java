 ...
 */
public class TesteRedesNeuraisResistor {
    public static void main(String arg[]){
        ...
        if(criar){
            ...
            try {
                DataSink.write("redes.arff", db);
            } catch (Exception ex) {
                Logger.getLogger(TesteRedesNeuraisResistor.class.getName()).log(Level.SEVERE, null, ex);
            }
        } else {
            try {
                db = DataSource.read("redes.arff"); // Trocar o nome da relação
            } catch (Exception ex) {
                Logger.getLogger(TesteRedesNeuraisResistor.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
        ...
        try {
            ...
            while(Integer.valueOf(JOptionPane.showInputDialog("0-sair\n1-classificar"))==1){
                ...
                dd2[0] = Math.random()*0.6+1.3;
                dd2[1] = Math.random()*80.0+25.0;
                String cor1 = JOptionPane.showInputDialog("Qual a primeira cor?");
                dd2[2] = db.attribute(2).indexOfValue(cor1);
                Instance teste = db.firstInstance().copy(dd2);
                JOptionPane.showConfirmDialog(null, "A pessoa do sexo "+db.attribute(2).value((int)dd2[2])+" que tem "+dd2[0]+"m, "+dd2[1]+"kg é considerado:");
